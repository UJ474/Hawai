...
import { Seat, SeatType } from "./Seat.js";
...
export class AirlineManagementSystem {
  ...
  addFlight(
    ...
    for (let i = 1; i <= capacity; i++) {
      ...
      const seat = new Seat(`${row}${col}`, seatType);
      flight.addSeat(seat);
    }
  ...
  bookFlight(
    ...
    const availableSeats = flight.getAvailableSeats();
    const seat = availableSeats.find((s) => s.getSeatNumber() === seatNumber);
